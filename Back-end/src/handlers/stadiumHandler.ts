  ...
  try {
    ...
    fs.unlinkSync(
      path.join(
        __dirname,
        '../../',
        process.env.STATIC_FILES_LOCATION as string,
        data.imageURL
      )
    );
    res.status(200);
...
const stadiumRoutes = (app: Application) => {
  ...
  app.post(
    '/stadium',
    [tokenVerfication, adminAuthorization, upload.single('stadiumImage')],
    create
  );
  app.put('/stadium', [tokenVerfication, adminAuthorization], edit);
  app.delete('/stadium/:id', [tokenVerfication, adminAuthorization], _delete);
};
