...
const MatchesTab = ({ currentTab }) => {
  ...
  const confirmWindowFunction = async (matchId) => {
    await matchController.remove(matchId);
    await refreshMatchList();
    setIsOpen(false);
  };

  const refreshMatchList = async () => {
    const matchList = await matchController.getMatchesWithStadium();
  ...
  return (
    <div className={' '.concat(currentTab === 1 ? '' : 'hidden')}>
      ...
      <MatchForm
        ...
        callbackFunction={refreshMatchList}
      />
      <MatchEditForm
        isOpen={isUpdateFormOpen}
        handleClose={handleUpdateFormClose}
        callbackFunction={refreshMatchList}
        matchData={currentSelectedMatch}
      />
              ...
              <div className="inner-container">
                ...
                <div className="body">
                  <div className="match-st-card">
                    <img
                      src={`${flagSrc}${match.firstTeam.toLowerCase()}.png`}
                      alt={match.firstTeam}
                    />
                    <p>{match.firstTeam}</p>
                  </div>
                  ...
                  <div className="match-nd-card">
                    <img
                      src={`${flagSrc}${match.secondTeam.toLowerCase()}.png`}
